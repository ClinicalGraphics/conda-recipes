...
with rt.Scene(800, 800, 45.0, 1e-13, 10) as scene, \
    ...
    for y in range(-2, 3):
        for x in range(-2, 3):
            scene.add_sphere(
                rt.point(x, y, -5.0),
                0.4,
                rt.material(next(colorations), next(surfaces), 0.18)
            )
    ...
    scene.render().save(file_path)

print("testing that png file was generated...")

# Check that a png file was successfully created
if not os.path.exists(file_path):
    raise Exception("Failed to create the png file")
